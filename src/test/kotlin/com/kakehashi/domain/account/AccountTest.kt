 ...
 * 《テスト》PROVISIONAL から ACTIVE に遷移し version が増加する
 *
 * 《観　点》editName: 表示名更新と変更追跡（version インクリメント）の確認
 * 《テスト》表示名を更新し version が増加する
 *
 * 《観　点》suspend: ACTIVE → SUSPENDED 遷移と停止日時記録の確認
 * 《テスト》ACTIVE から SUSPENDED に遷移し suspendedAt が設定される
 *
 * 《観　点》unsuspend: SUSPENDED → ACTIVE 遷移と停止日時クリアの確認
 * 《テスト》SUSPENDED から ACTIVE に遷移し suspendedAt がクリアされる
 *
 * ★★異常系★★
 * 《観　点》register: PROVISIONAL 以外からの登録を禁止（重複登録・誤操作の防止）
 * 《テスト》PROVISIONAL 以外から register を呼ぶと例外をスローする
 *
 * 《観　点》editName: 空文字列による表示名の空欄化を防止
 * 《テスト》空文字列を渡すと例外をスローする
 *
 * 《観　点》suspend: ACTIVE 以外からの停止操作を禁止（不正な状態変更の防止）
 * 《テスト》ACTIVE 以外から suspend を呼ぶと例外をスローする
 *
 * 《観　点》unsuspend: SUSPENDED 以外からの解除操作を禁止（不正な状態変更の防止）
 * 《テスト》SUSPENDED 以外から unsuspend を呼ぶと例外をスローする
 */
class AccountTest {
