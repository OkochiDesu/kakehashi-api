 ...
 *
 * 《観　点》境界値・採番ロジック（fromSequence）の確認
 * 《テスト》正常系： AZ0001 形式は有効
 * 《テスト》正常系： AZ9999 形式は有効
 * 《テスト》正常系： fromSequence でゼロ埋め4桁の AccountId が生成される
 *
 * 《観　点》プレフィックス・桁数・文字種のバリデーション境界の確認
 * 《テスト》異常系： プレフィックスなし（数字のみ）は不正フォーマット
 * 《テスト》異常系： 数字が5桁は不正フォーマット
 * 《テスト》異常系： 数字が3桁は不正フォーマット
 * 《テスト》異常系： プレフィックスが小文字は不正フォーマット
 * 《テスト》異常系： 空文字は不正フォーマット
 */
class AccountIdTest {
    @Test
