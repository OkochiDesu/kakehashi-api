...
5. 標準セクションが存在する（ステータス/日付/関連/背景/決定/代替案/影響/今後の見直しポイント）。**この項目は本文中の見出し存在のみを対象とし、`## 目次` 内のリンク網羅性は含めない。**
6. `docs/adr/README.md` の「ADR一覧（カテゴリ別索引）」テーブルに行が追加されているか。

### チェック対象外（指摘しないこと）

- `## 目次` 内のリンク網羅性（どの見出しをToCに含めるか）は本エージェントの検証対象外。特に**「日付」セクションへのToCリンク省略は既知の慣習であり漏落として指摘しない**（[.claude/rules/adr-rules.md](../rules/adr-rules.md#目次tocの書き方) 参照）
- ToCセクション自体の有無チェックは `doc-maintainer-structure` の担当であり、本エージェントの対象外
