...
5. **exec-plans整合性**: [docs/exec-plans/](../../docs/exec-plans/README.md) の各計画（pending/ / active/ / completed/）が、命名規則（4桁連番）に従っているか。「進捗状況」のチェック状態が、リポジトリの実際の状態（関連ファイルの有無など）と矛盾していないか。完了済みなのに `active/` に残っている計画がないか。`pending/` に exec-plan 昇格基準を満たしていない TODO レベルの項目が混入していないか。
