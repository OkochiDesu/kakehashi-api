...
5. **目次（ToC）の有無チェック**: `docs/` 配下の Markdown ファイルのうち、80行を超えるものに `## 目次` セクションが存在するかを確認する。存在しない場合は「要対応」として報告し、追加すべき見出しリンクの案を提示する。ただし以下のパスは除外する:
   ...
   **本チェックは `## 目次` セクションの有無のみを対象とし、既存ToC内のリンク網羅性（全見出しを含んでいるか）は対象外とする。** 特にADR（`*-ADR-*.md`）で「日付」セクションへのToCリンクが省略されている場合は既知の慣習であり指摘しない（[.claude/rules/adr-rules.md](../rules/adr-rules.md#目次tocの書き方) 参照）。
...
- 索引の網羅性: OK / 要対応
- リンク整合性: OK / 要対応
- .claude/構成との整合性: OK / 要対応
- サブディレクトリ内のファイル網羅性: OK / 要対応 / SKIP（対象ディレクトリなし）
- ToC有無: OK / 要対応 / SKIP（80行未満のファイルのみ）
