...
### exec-planの「準備」と「実行」を混同しない
- exec-plan を `pending/` から `active/` へ移動する、AskUserQuestion で次に着手する exec-plan・タスクを選定する、といった行為は「準備」であり、それ自体はサブエージェント起動によるファイル作成・コード実装などの「実行」を許可したことを意味しない
- AskUserQuestion の選択肢文言（例:「最初のADRを作成（推奨）」）への回答を得ただけの状態で、サブエージェントを起動し成果物（ADRドラフト・実装コード等）の作成に着手しない。「これから〇〇エージェントを起動し、〇〇を作成します。進めてよいですか」と実行対象を明示し、着手可否の確認を別途得てから実行する
- 判断に迷う場合は「準備（exec-planの選定・active化）はここまで完了しました。続けて実行（〇〇エージェントの起動）に進んでよいですか」と一度区切って確認する
...
- `git commit` の前に、現在のブランチに対応するPRの状態を `gh pr view --json state,number` で確認する
  ...
  - ただし、未マージ（`OPEN`等）のPRが存在する状態で、そのPRのスコープ外の別exec-plan・別タスクに着手する場合は、機械的に同じブランチで作業を続けず、続けて同じブランチで作業してよいか・新しいブランチを切るべきかをユーザーに確認すること（新規ブランチの場合は `git checkout -b feature/<内容> origin/main` で作成する）
