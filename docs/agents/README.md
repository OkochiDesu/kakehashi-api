...
### レビュー系エージェント（APPROVED / REQUIRES_CHANGES を返すもの）の追加ルール

**`## 出力フォーマット` には全チェック項目を `PASS / FAIL / SKIP` で必ず列挙すること**（根拠: [AI-ADR-0018](../adr/AI-ADR-0018-レビュー系エージェントの全項目列挙出力パターン.md)）。

- `PASS`: チェックして問題なし
- `FAIL`: 問題あり（指摘事項セクションで詳述）
- `SKIP`: diff の内容上、当該チェックが対象外

問題項目のみの列挙は禁止。チェックをスキップしても出力に現れないため、最終ゲートとしての信頼性が失われる。
