> 出典: https://openai.com/ja-JP/index/harness-engineering/

## 目次

- [最初は空の Git リポジトリから](#最初は空の-git-リポジトリから)
- [エンジニアの役割の再定義](#エンジニアの役割の再定義)
- [アプリケーションの可読性向上](#アプリケーションの可読性向上)
- [リポジトリの知識を記録システムの基盤に](#リポジトリの知識を記録システムの基盤に)
- [エージェントの認識可能性が目標](#エージェントの認識可能性が目標)
- [アーキテクチャと好みの適用](#アーキテクチャと好みの適用)
- [スループットがマージの考え方を変える](#スループットがマージの考え方を変える)
- [「エージェント生成」が実際に意味するもの](#エージェント生成が実際に意味するもの)
- [自律性レベルの向上](#自律性レベルの向上)
- [エントロピーとガベージコレクション](#エントロピーとガベージコレクション)
- [私たちが今なお学んでいること](#私たちが今なお学んでいること)
...
過度なガイダンスは指針を失う。すべてが「重要」であるときには、何も重要ではなくなります。エージェントは意図的にナビゲートするのではなく、局所的にパターンマッチングを行うことに終始することになります。
...
リポジトリは完全にエージェントによって生成されるため、まず Codex の認識可能性が最適化されています。チームが新規採用エンジニア向けにコードの操作性を向上させるのと同じように、私たち人間のエンジニアの目標は、エージェントがリポジトリ自体から直接ビジネスドメイン全体について推論できるようにすることでした。
...
[エージェントは厳格な境界と予測可能な構造](https://bits.logic.inc/p/ai-is-forcing-us-to-write-good-code)を持つ環境で最も機能を発揮するため、私たちはアプリケーションを厳密なアーキテクチャモデルに基づいて構築しました。各ビジネスドメインは固定されたレイヤー群に分割され、依存関係の方向性は厳密に検証され、許容されるエッジは限定された集合で構成されます。これらの制約は、カスタムリンター（もちろん Codex 生成です）と構造テストによって機械的に適用されます。
