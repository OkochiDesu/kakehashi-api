# CICD-ADR-0004: コミットメッセージベースのPRサマリー自動コメント導入
...
注: 決定4（コメント投稿方式）は [CICD-ADR-0005](CICD-ADR-0005-PR本文への変更内容自動反映方式への変更.md) によりPR本文への直接反映方式に置き換えられた。決定1〜3は引き続き有効。
...
- Supersedes: なし
- Superseded by: [CICD-ADR-0005](CICD-ADR-0005-PR本文への変更内容自動反映方式への変更.md)（決定4「コメント投稿方式」のみ）
...
- `ci.yml`（CICD-ADR-0001/ADR-0002のverify/reports構成）には組み込まず、独立したワークフローとする
...
- CICD-ADR-0001/ADR-0003と同様、外部ActionはSHA固定で利用する
