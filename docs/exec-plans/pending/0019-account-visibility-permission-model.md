...
- 2026-07-11: PR #19 のレビューコメントを起点に、`account-domain-agent` で既存ADR（APP-ADR-0007等）との整合性を検証。`view_personal_info` は現行のAPP-ADR-0007では経歴書ドメイン限定で決定されており、アカウント情報への適用可否はこのADRの決定範囲外＝**未決定**であることを確認した（「適用しない」という結論ではない）。ユーザーとの対話で、アカウント情報にも適用する想定の権限マトリクス（`admin`/`view_personal_info`/本人/他人）を確定し、正式な適用範囲の決定は進捗状況の「ADR作成」タスクで行う。
