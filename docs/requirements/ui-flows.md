...
- **未確定事項**: 「技術部門の方経由とするか、直接にするか」という論点があり、エンジニア間の直接コミュニケーションが難しい場合に営業担当を介する運用を検討中。営業を介する場合、コンタクト先・送信フローに営業担当の選択ステップが追加される可能性がある（要追加確認）。
...
- **未確定事項**: 「（検索された）マスク有済み経歴書」（本人以外が閲覧する際のマスク対象項目）の範囲は未確定。`visibility_rules.target_category`との対応（[data-models.md 4章](data-models.md#4-経歴書コンテキスト)）はUC-R2のQuery実装時に確認する。
